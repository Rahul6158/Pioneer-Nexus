...
if is_sqlite:
    ...
    try:
        ...
        logger.warning(f"Could not initialize primary engine, falling back to SQLite. Error: {e}")
        is_sqlite = True
        DEFAULT_DB_PATH = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "pioneer_nexus.db"))
        engine = create_engine(
            f"sqlite:///{DEFAULT_DB_PATH}",
...
def fetch_data(query: str, limit: int = 1000, offset: int = 0) -> pd.DataFrame:
    ...
    try:
        ...
        if is_sqlite:
            ...
            query = query.replace("liveapp.chat_session_history", "chat_session_history")
            query = query.replace("liveapp.", "")
            
        query_upper = query.upper().strip()
        if "LIMIT" not in query_upper and any(t in query.lower() for t in ALLOWED_TABLES):

...
if not GEMINI_API_KEY:
    logger.error("GEMINI_API_KEY not found in environment variables. AI features will use fallback mock summaries.")
...
def validate_sql_safe(sql: str) -> bool:
    ...
    for kw in destructive_keywords:
        if re.search(r'\b' + kw + r'\b', sql_upper):
            logger.error(f"Security Block: Destructive keyword '{kw}' detected in query.")
            return False
...
def run_expiry_match_engine_task():
    ...
    try:
        # 1. Try Live Database
        df = fetch_data("SELECT * FROM liveapp.pharmasales LIMIT 5000") # Limit for faster processing
        ...
        if df.empty:
            ...
            csv_path = os.path.join(base_dir, "..", "pioneer_pharma_ideal_dataset_70000_rows.csv")
            if os.path.exists(csv_path):
                logger.info(f"Loading data from CSV fallback for engine: {csv_path}")
                df = pd.read_csv(csv_path)
            else:
                logger.warning("Engine Warning: No data source available (DB empty & CSV missing).")
                return
        ...
        logger.info(f"Expiry-to-Demand Match Engine Analysis complete. Found {len(results_df)} batches at risk.")
    ...
    try:
        with engine.begin() as conn:
            ...
            for col, col_type in columns_to_add.items():
                try:
                    conn.execute(text(f"ALTER TABLE liveapp.chat_session_history ADD COLUMN IF NOT EXISTS {col} {col_type}"))
                except: pass
                
            logger.info("chat_session_history table verified/updated with metrics columns.")
    except Exception as e:
...
async def get_descriptive_summary(req: SummaryRequest):
    ...
    cache_input = f"descriptive_{req.view}_{req.language}_{json.dumps(req.kpis, sort_keys=True)}"
    ...
    try:
        ...
        return {"summary": "The Descriptive Analysis module provides a data-driven view of Pioneer Pharma's operations. Analyzing ASP trends, geographic distribution, and warehouse intensity helps stakeholders identify growth opportunities."}
...
async def get_predictive_summary(req: PredictiveSummaryRequest):
    ...
    try:
        ...
        return {"summary": "Our predictive models indicate continued volatility in the pharmaceutical supply chain. Strategic focus is required on managing expiry risks while actively supporting high-demand product lines."}
...
async def copilot_chat(req: CopilotRequest, request: Request):
    try:
        ...
        if clean_msg in greetings or (len(clean_msg) < 15 and any(g in clean_msg for g in greetings)):
             if req.language == 'ar':
                 return {"response": "أهلاً بك! أنا Pioneer Nexus AI، محلل البيانات الدوائية الخاص بك. أنا متصل ببياناتك المباشرة. كيف يمكنني مساعدتك في تحليلاتك اليوم؟"}
             return {"response": "Hello! I am Konfig Nexus AI, your pharmaceutical analyst. I'm connected to your live data. How can I assist you with your analytics today?"}
...
"""
        ...
        product_list = [
            "CLAFONEER 500mg IV/IM Vial", "No-vomit 8mg/4ml Injection", "ATRANEER 10mg/ml Injection",
            "SITAVIA Plus 50/1000mg", "SITAVIA 100mg Tablets", "NAPRON 500mg Tablet",
            "Piodol 500mg Tablets", "LOSART 50mg Tablets", "Neuro-Forte F/C Tablet", "CLOXABAN 20mg F/C Tablet"
        ]
        ...
        completion = gemini_client.models.generate_content(model=GEMINI_MODEL, contents=sql_prompt)
        sql_response = completion.text.strip()
        sql_response = sql_response.replace("```sql", "").replace("```", "").strip().rstrip(';').strip()
        ...
        response = gemini_client.models.generate_content(model=GEMINI_MODEL, contents=final_prompt)
...
def get_dashboard_summary_data(request: Request, month: Optional[int] = None):
    ...
    def process_kpis():
        ...
        return convert_to_native({
            "total_revenue": {"value": rev or 15420000, "suffix": "IQD", "trend": 5.4},
            "total_units_sold": {"value": qty or 8540, "suffix": "Units", "trend": 3.1},
            "avg_selling_price": {"value": rev / qty if qty > 0 else 450.25, "suffix": "IQD", "trend": 0.5},
    ...
    def process_regional():
        reg = df.pivot_table(index='date', columns='region', values='revenue', aggfunc='sum').fillna(0).reset_index()
        reg['date'] = pd.to_datetime(reg['date']).dt.strftime('%b %d')
        return df_to_dict(reg)

    def process_top_products():
        top = df.groupby('product_name').agg({'quantity': 'sum', 'revenue': 'sum'}).sort_values('quantity', ascending=False).head(5).reset_index()
    ...
    def process_wh_volume():
        vol = df.groupby('warehouse').agg({'quantity': 'sum', 'orders': 'sum'}).reset_index()
    ...
    with ThreadPoolExecutor(max_workers=5) as executor:
        futures = {
            ...
            "revenueByRegion": executor.submit(lambda: df_to_dict(df.groupby('region')['revenue'].sum().reset_index().rename(columns={'revenue': 'revenue'}))),
            ...
            "batchPerformance": executor.submit(lambda: df_to_dict(df.groupby('product_name')['revenue'].sum().reset_index().rename(columns={'product_name': 'batch'}).head(10))),
...
def get_kpis(request: Request, month: Optional[int] = None):
    ...
    try:
        ...
        if combined_df.empty:
            ...
            revenue = combined_df['total_revenue'].iloc[0] if pd.notna(combined_df['total_revenue'].iloc[0]) else 15420000
            units = combined_df['total_units'].iloc[0] if pd.notna(combined_df['total_units'].iloc[0]) else 8540
            returns = combined_df['return_count'].iloc[0] if pd.notna(combined_df['return_count'].iloc[0]) else 0
            
            res = convert_to_native({
                "total_revenue": {"value": revenue, "suffix": "IQD", "trend": 5.4},
                "total_units_sold": {"value": units, "suffix": "Units", "trend": 3.1},
                "avg_selling_price": {"value": revenue / units if units > 0 else 450.25, "suffix": "IQD", "trend": 0.5},
                "active_models": {"value": 6, "suffix": "AI Engines", "trend": 20.0},
...
def get_revenue_trend():
    ...
    df = fetch_data("SELECT DATE(order_date) as date, SUM(revenue_iqd) as revenue FROM liveapp.pharmasales GROUP BY DATE(order_date) ORDER BY date")
    if df.empty:
        days = [(datetime.now() - timedelta(days=i)).strftime('%b %d') for i in range(30, 0, -1)]
...
def get_regional_sales_trend():
    ...
    df = fetch_data("SELECT DATE(order_date) as date, region, SUM(revenue_iqd) as total_price FROM liveapp.pharmasales GROUP BY DATE(order_date), region")
    if df.empty:
        days = [(datetime.now() - timedelta(days=i)).strftime('%b %d') for i in range(10, 0, -1)]
        regions = ["Baghdad", "Basra", "Erbil", "Mosul"]
        data = []
        for d in days:
            entry = {"date": d}
            for r in regions: entry[r] = random.randint(100000, 300000)
            data.append(entry)
        res = data
    else:
        df['date'] = pd.to_datetime(df['date']).dt.strftime('%b %d')
        pivot = df.pivot_table(index='date', columns='region', values='total_price', aggfunc='sum').fillna(0).reset_index()
...
def get_top_products():
    ...
    df = fetch_data("SELECT product_name as name, SUM(quantity) as quantity, SUM(revenue_iqd) as revenue FROM liveapp.pharmasales GROUP BY product_name ORDER BY quantity DESC LIMIT 5")
    if df.empty:
        res = [{"name": f"Product {i}", "quantity": random.randint(100, 1000), "revenue": random.randint(50000, 500000)} for i in range(5)]
    else:
...
def get_revenue_by_region():
    ...
    df = fetch_data("SELECT region, SUM(revenue_iqd) as revenue FROM liveapp.pharmasales GROUP BY region")
    if df.empty:
        res = [{"region": r, "revenue": random.randint(1000000, 5000000)} for r in ["Baghdad", "Basra", "Erbil", "Mosul", "Anbar"]]
    else:
        res = df.to_dict(orient="records")
...
def get_warehouse_performance():
    ...
    df = fetch_data("SELECT warehouse, SUM(revenue_iqd) as sales FROM liveapp.pharmasales GROUP BY warehouse")
    if df.empty:
        res = [{"warehouse": w, "sales": random.randint(2000000, 5000000), "capacity": random.randint(60, 95)} for w in ["Central", "South", "North"]]
    else:
...
def get_warehouse_volume():
    ...
    df = fetch_data("SELECT warehouse, SUM(quantity) as volume, COUNT(DISTINCT order_id) as orders FROM liveapp.pharmasales GROUP BY warehouse")
    if df.empty:
...
def get_product_contribution():
    ...
    df = fetch_data("SELECT product_name as name, SUM(revenue_iqd) as value FROM liveapp.pharmasales GROUP BY product_name")
    if df.empty:
        res = [{"name": c, "value": random.randint(10, 40)} for c in ["Antibiotics", "Painkillers", "Cardio", "Diabetic"]]
    else:
        total = df['value'].sum()
...
def get_revenue_by_type():
    ...
    df = fetch_data("SELECT transaction_type as type, SUM(revenue_iqd) as revenue FROM liveapp.pharmasales GROUP BY transaction_type")
    if df.empty:
        res = [{"type": t, "revenue": random.randint(1000000, 5000000)} for t in ["Direct", "Bulk", "Return"]]
    else:
        res = df.to_dict(orient="records")
...
def get_batch_performance():
    ...
    df = fetch_data("SELECT batch_id as batch, SUM(revenue_iqd) as revenue FROM liveapp.pharmasales GROUP BY batch_id LIMIT 10")
    if df.empty:
        res = [{"batch": f"B-{i}", "revenue": random.randint(500000, 1500000)} for i in range(5)]
    else:
        res = df.to_dict(orient="records")
...
def get_demand_distribution():
    ...
    if df.empty:
        ...
        res = [{"bin": f"{int(bins[i])}-{int(bins[i+1])}", "count": int(counts[i])} for i in range(len(counts))]
...
def get_inventory():
    ...
    df = fetch_data("SELECT * FROM liveapp.pharmasales ORDER BY created_at DESC LIMIT 100")
    if df.empty:
        res = []
    else:
        res = df_to_dict(df)
...
class MLModels:
        ...
        if cls.demand_model is None:
            with cls._lock:
                if cls.demand_model is None:
                    ...
                    base_dir = os.path.dirname(os.path.abspath(__file__))
                    model_dir = os.path.join(base_dir, "..", "ML_trained_ouputs", "geo_demand_forecasting")
                    ...
                    if all(os.path.exists(p) for p in [model_path, prod_enc_path, reg_enc_path]):
                        try:
                            ...
                            logger.info(f"Loaded pre-trained Demand model from {model_dir}")
                        except Exception as e:
    ...
    def get_sales_model(cls):
        if cls.sales_model is None:
            with cls._lock:
                if cls.sales_model is None:
                    base_dir = os.path.dirname(os.path.abspath(__file__))
                    model_path = os.path.join(base_dir, "..", "ML_trained_ouputs", "financial_sales_forecasting_training", "revenue_forecast_model1.pkl")
                    if os.path.exists(model_path):
                        try:
                            cls.sales_model = joblib.load(model_path)
                            cls.is_prophet = True
                            logger.info(f"Loaded pre-trained Prophet model from {model_path}")
                        except Exception as e:
    ...
    def get_return_model(cls):
        if cls.return_model is None:
            with cls._lock:
                if cls.return_model is None:
                    base_dir = os.path.dirname(os.path.abspath(__file__))
                    model_dir = os.path.join(base_dir, "..", "ML_trained_ouputs", "return_probability_classifier")
                    model_path = os.path.join(model_dir, "return_probability_model.pkl")
                    prod_enc_path = os.path.join(model_dir, "return_product_encoder.pkl")
                    reg_enc_path = os.path.join(model_dir, "return_region_encoder.pkl")

                    if all(os.path.exists(p) for p in [model_path, prod_enc_path, reg_enc_path]):
                        try:
                            cls.return_model = joblib.load(model_path)
                            cls.label_encoders['return_product'] = joblib.load(prod_enc_path)
                            cls.label_encoders['return_region'] = joblib.load(reg_enc_path)
                            logger.info(f"Loaded pre-trained Return model from {model_dir}")
                        except Exception as e:
    ...
    def train_demand_model(cls):
        ...
        df['product_encoded'] = le_product.fit_transform(df['product_name'].fillna('Unknown'))
    ...
    def train_sales_model(cls):
        ...
        daily_sales = df.groupby(['days_since_start', 'region']).agg({
            'total_price': 'sum',
            'month': 'first',
            'day_of_week': 'first'
        }).reset_index()
        ...
        le_region = LabelEncoder()
        daily_sales['region_encoded'] = le_region.fit_transform(daily_sales['region'].fillna('Unknown'))
...
def predict_demand(req: DemandPredictionRequest):
    ...
    try:
        ...
        """)
        confidence = min(0.95, 0.7 + (df_count['count'].iloc[0] / 500) if not df_count.empty else 0.7)
...
def get_sales_forecast(days: int = 30, region: Optional[str] = None):
    ...
    try:
        ...
        last_date_df = fetch_data("SELECT MAX(order_date) as last_date FROM liveapp.pharmasales")
        base_date = pd.to_datetime(last_date_df['last_date'].iloc[0]) if not last_date_df.empty and last_date_df['last_date'].iloc[0] else datetime.now()
        
        # Region setup
        requested_region = region if region and region != "All Regions" else None
        regions = [requested_region] if requested_region else ["Baghdad", "Basra", "Erbil", "Mosul", "Anbar"]
        
        forecast_df = None
        if model and is_prophet:
            try:
                future = pd.DataFrame({'ds': [base_date + timedelta(days=i) for i in range(1, days + 1)]})
        ...
        for r in regions:
            scale = {"Baghdad": 0.22, "Basra": 0.19, "Mosul": 0.21, "Erbil": 0.20, "Anbar": 0.18}.get(r, 0.20)
            f_list = []
            for i in range(1, days + 1):
                f_date = base_date + timedelta(days=i)
                region_map_id = {"Baghdad": 1, "Basra": 2, "Erbil": 3, "Mosul": 4, "Anbar": 5}.get(r, 0)
                rng = random.Random(region_map_id * 1000 + f_date.toordinal())
                
                if forecast_df is not None and i-1 < len(forecast_df):
                    base_val = forecast_df.iloc[i-1]['yhat'] * (scale / total_scale_sum)
                    reg_volatility = {"Baghdad": 1.2, "Basra": 0.9, "Erbil": 1.1, "Mosul": 1.0, "Anbar": 0.8}.get(r, 1.0)
                    spike_chance = rng.random()
                    multiplier = rng.uniform(1.5, 2.1) if spike_chance > 0.94 else rng.uniform(1.2, 1.4) if spike_chance > 0.80 else rng.uniform(0.85, 1.15)
                    val = max(base_val * multiplier * reg_volatility, 0)
                else:
                    reg_volatility = {"Baghdad": 1.25, "Basra": 0.85, "Erbil": 1.15, "Mosul": 1.0, "Anbar": 0.75}.get(r, 1.0)
                    base_val = (rng.randint(2500000, 3500000) * scale) * reg_volatility
                    spike_chance = rng.random()
                    multiplier = rng.uniform(1.7, 2.3) if spike_chance > 0.94 else rng.uniform(1.2, 1.5) if spike_chance > 0.82 else rng.uniform(0.85, 1.25)
        ...
        result = convert_to_native({
            ...
            "summary": {
                ...
                "total_predicted_revenue": round(sum(f['predicted_revenue'] for f in forecasts), 2),
                "avg_daily_revenue": round(sum(f['predicted_revenue'] for f in forecasts) / days, 2),
                "trend": "upward"
            }
        })
        predictive_cache[cache_key] = result
...
def get_expiry_risk_prediction():
    ...
    try:
        base_dir = os.path.dirname(os.path.abspath(__file__))
        model_output = os.path.join(base_dir, "..", "ML_trained_ouputs", "expiry_risk_detection", "expiry_prediction_output.pkl")
        
        if not os.path.exists(model_output):
            logger.warning(f"ML Output not found at {model_output}. Falling back to empty.")
        ...
        summary = {
            ...
            "estimated_potential_loss": float(future_risks['potential_value_loss'].sum()),
        ...
        result = convert_to_native({
            ...
            ] if len(future_risks) > 0 else ["No immediate expiry risks detected"]
        })
        ...
        return convert_to_native({
            ...
            "recommendations": ["Internal Error: ML Pipeline output could not be read"]
        })
...
def get_return_probability_prediction():
    ...
    try:
            ...
            if le_prod and le_reg:
                ...
                try:
                    ...
                    encoded_data = pd.DataFrame({
                        'region_encoded': df['region'].apply(lambda x: le_reg.transform([x])[0] if x in le_reg.classes_ else -1),
                        'product_encoded': df['product'].apply(lambda x: le_prod.transform([x])[0] if x in le_prod.classes_ else -1),
                        'quantity': pd.Series(df.get('total_transactions', [100]*len(df))).fillna(100).astype(float),
                        'unit_price_iqd': pd.Series(df.get('avg_return_value', [5000]*len(df))).fillna(5000).astype(float)
                    })
                    ...
                    for i, prob in enumerate(probs):
                        ...
                        predictions.append({
                            ...
                            "risk_level": "High" if prob >= 10 else "Medium" if prob >= 5 else "Low"
                        })
                except Exception as e:
                    logger.error(f"Vectorized Return Prediction Error or Fallback failed: {e}")
                    # Minimal fallback to statistical prob if everything else fails
                    predictions = []
                    for row in df.itertuples():
                        stats_prob = (row.returns / row.total_transactions * 100) if getattr(row, 'total_transactions', 0) > 0 else 0
                        predictions.append({
                            "product": row.product,
                            "region": row.region,
                            "warehouse": getattr(row, 'warehouse', 'N/A'),
                            "returns": int(getattr(row, 'returns', 0)),
                            "total_transactions": int(getattr(row, 'total_transactions', 0)),
                            "return_probability": round(float(stats_prob), 2),
                            "risk_level": "High" if stats_prob >= 10 else "Medium" if stats_prob >= 5 else "Low"
                        })
                df = pd.DataFrame(predictions)
        elif df.empty:
            # Generate mock return probability data
            products = ["SITAVIA", "ATRANEER", "MEDIFLEX", "BIOTICIN", "PAINRELIEF", "CARDIOCARE"]
            regions = ["Baghdad", "Basra", "Erbil", "Mosul"]
        ...
        if not df.empty and 'returns' in df.columns:
            ...
            avg_return_rate = (total_returns / total_trans * 100) if total_trans > 0 else 0
        ...
        return convert_to_native({
            ...
            ] if total_returns > 0 else ["No significant return patterns detected"]
        })
    except Exception as e:
...
def get_all_predictions():
    ...
    try:
        ...
        if top_products.empty:
            mock_products = ['SITAVIA', 'ATRANEER', 'MEDIFLEX', 'BIOTICIN', 'PAINRELIEF']
            mock_regions = ['Baghdad', 'Basra', 'Erbil']
            top_products = pd.DataFrame([
                {'product_name': random.choice(mock_products), 'region': random.choice(mock_regions)}
                for _ in range(5)
            ])
        ...
        if not top_products.empty:
            ...
            if model and le_prod and le_reg:
                ...
                for row in top_products.itertuples():
                    try:
                        p_enc = le_prod.transform([row.product_name])[0] if row.product_name in le_prod.classes_ else 0
                        r_enc = le_reg.transform([row.region])[0] if row.region in le_reg.classes_ else 0
                        batch_data.append({
...
def api_get_recommendations():
    ...
    try:
        ...
        df = fetch_data("SELECT product_name, batch_id, quantity, region, expiry_date, order_date FROM liveapp.pharmasales LIMIT 2000")
        
        if df.empty:
            return convert_to_native({
                "inventory": [], 
                "warehouse": {"recommended_region": "Unknown", "reason": "No data available"}, 
                "expiry": []
            })
        ...
        products_df = fetch_data("SELECT product_name FROM liveapp.pharmasales GROUP BY product_name LIMIT 2")
        prod1 = products_df['product_name'].iloc[0] if len(products_df) > 0 else "SITAVIA"
        prod2 = products_df['product_name'].iloc[1] if len(products_df) > 1 else "ATRANEER"
        ...
        return convert_to_native({
            "error": str(e), 
            "inventory": [], 
            "warehouse": {"recommended_region": "Unknown", "reason": "Error generating recommendations"}, 
            "expiry": []
        })
...
def api_copilot(req: CopilotQueryRequest, request: Request):
    ...
    try:
        ...
        return convert_to_native({"type": "error", "message": "The assistant encountered an internal error: " + str(e)})
...
def get_database_tables(table_name: str = "pharmasales", page: int = 1, page_size: int = 10):
    """
    ...
    if table_name not in allowed:
        raise HTTPException(status_code=400, detail="Invalid or unauthorized table request")
            ...
            for line in result.stdout.splitlines():
                if f":{port}" in line and 'LISTENING' in line:
                    ...
                    if len(parts) >= 5:
                        pid = parts[-1]
                        if pid != "0":
                            logger.warning(f"Port {port} is currently used by PID {pid}. Terminating it automatically...")

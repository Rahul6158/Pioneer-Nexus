...
def evaluate_ml_models():
    ...
    try:
        ...
        if os.path.exists(model_path):
            ...
            df = fetch_data("SELECT DATE(order_date) as ds, SUM(revenue_iqd) as y FROM liveapp.pharmasales GROUP BY DATE(order_date) ORDER BY ds")
            ...
            if not df.empty and len(df) > 10:
                ...
                logger.info(f"Prophet Forecast Model: MAPE={mape:.2f}%, RMSE={rmse:.2f}")
    ...
    with open(forecasting_metrics_path, mode='w', newline='', encoding='utf-8') as f:
        writer = csv.writer(f)
        writer.writerow(["model_name", "mape_percent", "rmse_iqd", "mae_iqd", "samples_evaluated", "status"])
        for row in forecast_metrics:
            writer.writerow([row["metric_name"], row["mape"], row["rmse"], row["mae"], row["samples_evaluated"], row["status"]])

    # 2. Risk Prediction Classifiers (Random Forest Return & Expiry)
    risk_metrics = []
    try:
        from sklearn.metrics import precision_recall_fscore_support
        model_path = os.path.join(base_dir, "ML_trained_ouputs", "return_probability_classifier", "return_probability_model.pkl")
        prod_enc_path = os.path.join(base_dir, "ML_trained_ouputs", "return_probability_classifier", "return_product_encoder.pkl")
        reg_enc_path = os.path.join(base_dir, "ML_trained_ouputs", "return_probability_classifier", "return_region_encoder.pkl")
        ...
        if all(os.path.exists(p) for p in [model_path, prod_enc_path, reg_enc_path]):
            ...
            df = fetch_data("SELECT product_name, region, quantity, unit_price_iqd, transaction_type FROM liveapp.pharmasales")
            
            if not df.empty and len(df) > 20:
                df['y_true'] = (df['transaction_type'] == 'Return').astype(int)
                
                df['region_encoded'] = df['region'].apply(lambda x: le_reg.transform([x])[0] if x in le_reg.classes_ else -1)
                df['product_encoded'] = df['product_name'].apply(lambda x: le_prod.transform([x])[0] if x in le_prod.classes_ else -1)
                
                # Filter out unseen labels
                val_df = df[(df['region_encoded'] != -1) & (df['product_encoded'] != -1)]
                
                if len(val_df) > 5:
                    features = ['region_encoded', 'product_encoded', 'quantity', 'unit_price_iqd']
                    ...
                    logger.info(f"Return Classifier RF: F1={f1:.4f}, Precision={precision:.4f}")
    except Exception as e:
    ...
    with open(risk_model_metrics_path, mode='w', newline='', encoding='utf-8') as f:
        writer = csv.writer(f)
        writer.writerow(["model_name", "precision", "recall", "f1_score", "samples_evaluated", "status"])
        for row in risk_metrics:
            writer.writerow([row["model_name"], row["precision"], row["recall"], row["f1_score"], row["samples_evaluated"], row["status"]])
...
def evaluate_text_to_sql():
    ...
    if not api_key:
        logger.warning("GEMINI_API_KEY is not configured in .env. Authentic benchmark CSVs will be generated with headers only, as per Authenticity Policy.")
        
        # Write empty CSV files with headers as per policy
        with open(copilot_accuracy_path, mode='w', newline='', encoding='utf-8') as f:
            csv.writer(f).writerow(["query_id", "question", "syntax_valid", "execution_success", "semantic_match", "latency_ms"])
        with open(query_success_rate_path, mode='w', newline='', encoding='utf-8') as f:
            csv.writer(f).writerow(["category", "total_queries", "successful_executions", "success_rate"])
        with open(latency_metrics_path, mode='w', newline='', encoding='utf-8') as f:
            csv.writer(f).writerow(["query_id", "question", "parse_time_ms", "execution_time_ms", "total_latency_ms"])
    ...
    for item in gold_queries:
        ...
        try:
            ...
            generated_sql = response.text.strip().replace("```sql", "").replace("```", "").strip().rstrip(';')
            ...
            if generated_sql and "SELECT" in generated_sql.upper():
                ...
                execution_success = not df.empty or len(df) == 0 # Executed without throwing DB Exception
                
                # 3. Semantic correctness comparison
                # We normalize query comparisons by removing whitespaces, casing, and schema prefixes
                def normalize(s):
                    return "".join(s.lower().replace("liveapp.", "").replace("`", "").replace("\"", "").split())
            ...
            logger.info(f"QID {qid} - Success: {execution_success}, Semantic Match: {semantic_match}, Latency: {total_latency:.1f}ms")
    ...
    with open(copilot_accuracy_path, mode='w', newline='', encoding='utf-8') as f:
        writer = csv.writer(f)
        writer.writerow(["query_id", "question", "syntax_valid", "execution_success", "semantic_match", "latency_ms"])
        for r in copilot_results:
            writer.writerow([r["id"], r["question"], r["syntax_valid"], r["execution_success"], r["semantic_match"], r["total_latency_ms"]])
            
    # Write success rates by category
    categories = set(r["category"] for r in copilot_results)
    with open(query_success_rate_path, mode='w', newline='', encoding='utf-8') as f:
        writer = csv.writer(f)
        writer.writerow(["category", "total_queries", "successful_executions", "success_rate"])
        for cat in categories:
            cat_runs = [r for r in copilot_results if r["category"] == cat]
            successes = sum(r["execution_success"] for r in cat_runs)
            writer.writerow([cat, len(cat_runs), successes, round(successes / len(cat_runs) * 100, 2)])
            
    # Write latency breakdown
    with open(latency_metrics_path, mode='w', newline='', encoding='utf-8') as f:
        writer = csv.writer(f)
        writer.writerow(["query_id", "question", "parse_time_ms", "execution_time_ms", "total_latency_ms"])
        for r in copilot_results:
            writer.writerow([r["id"], r["question"], r["parse_time_ms"], r["execution_time_ms"], r["total_latency_ms"]])
...
def write_markdown_report(forecast, risk, copilot):
    ...
    with open(results_md_path, mode='w', encoding='utf-8') as f:
        f.write("# Pioneer Nexus: AI-Powered Analytics Copilot & Forecasting Evaluation Report\n\n")
        f.write(f"**Report Generated On:** {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n\n")
        
        f.write("## Executive Summary\n")
        f.write("This report validates the production capabilities of the Pioneer Nexus intelligence engines. ")
        f.write("All metrics are derived from actual execution logs and pre-trained ML model evaluations, adhering strictly to our Authenticity Policy.\n\n")
        ...
        if copilot:
            ...
            f.write("| QID | NL Question | Syntax | Exec | Semantic Match | Latency (ms) |\n")
            f.write("| --- | --- | --- | --- | --- | --- |\n")
            for r in copilot:
                f.write(f"| {r['id']} | {r['question']} | {'✅' if r['syntax_valid'] else '❌'} | {'✅' if r['execution_success'] else '❌'} | {'✅' if r['semantic_match'] else '❌'} | {r['total_latency_ms']:.1f} |\n")
        else:
            f.write("> [!WARNING]\n")
            f.write("> Gemini API credentials were not available during this execution run. Text-to-SQL copilot execution metrics have been left empty to prevent fabrication.\n\n")
        ...
        if forecast:
            ...
            f.write("| --- | --- | --- | --- | --- |\n")
            for r in forecast:
                f.write(f"| {r['metric_name']} | {r['mape']:.2f}% | {r['rmse']:,} | {r['mae']:,} | {r['samples_evaluated']} |\n")
        else:
            f.write("> [!NOTE]\n")
            f.write("> Prophet sales forecast model files were not found or historical data was empty. Forecasting metrics have been left empty.\n\n")
        ...
        if risk:
            ...
            f.write("| --- | --- | --- | --- | --- |\n")
            for r in risk:
                f.write(f"| {r['model_name']} | {r['precision']:.4f} | {r['recall']:.4f} | {r['f1_score']:.4f} | {r['samples_evaluated']} |\n")
        else:
            f.write("> [!NOTE]\n")
            f.write("> Return probability classifier model files were not found or testing datasets were empty. Risk model metrics have been left empty.\n\n")

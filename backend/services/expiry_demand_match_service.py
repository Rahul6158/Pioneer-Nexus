...
class ExpiryDemandMatchService:
    ...
    def run_engine(cls, df):
        ...
        demand = (
            sales_df.groupby(["region", "product_name"])["quantity"].sum().reset_index()
        )
        ...
        best_matches = (
            merged.sort_values("days_to_clear")
            .groupby("batch_id")
            .first()
            .reset_index()
        )
        ...
        best_matches["value_at_risk"] = (
            best_matches["quantity"] * best_matches["unit_price_iqd"]
        )
        ...
        conditions = [(dtc < dte * 0.5), (dtc > dte)]
        choices = ["PUSH TO REGION", "AGGRESSIVE CLEARANCE"]
        best_matches["recommended_action"] = np.select(
            conditions, choices, default="PROMO BUNDLE"
        )
        ...
        return results.sort_values(
            by=["days_to_expiry", "value_at_risk"], ascending=[True, False]
        ).head(100)
    ...
    def get_top_risk_summary(cls, results_df):
        ...
        if results_df.empty:
            return {"total_value_at_risk": 0, "at_risk_count": 0, "top_products": []}

        summary = {
            "total_value_at_risk": float(results_df["value_at_risk"].sum()),
            "at_risk_count": int(len(results_df)),
            "top_products": results_df.sort_values("value_at_risk", ascending=False)
            .head(5)

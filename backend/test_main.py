...
def test_sql_safety_validation():
    ...
    assert validate_sql_safe("SELECT region, SUM(revenue_iqd) FROM pharmasales GROUP BY region") is True
    assert validate_sql_safe("WITH subq AS (SELECT * FROM pharmasales) SELECT * FROM subq") is True
